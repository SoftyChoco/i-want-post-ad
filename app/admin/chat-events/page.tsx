        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 text-left text-gray-600 bg-gray-50">
              <th className="py-3 px-4 font-medium">관찰시각 (KST)</th>
              <th className="py-3 px-4 font-medium">작성자</th>
              <th className="py-3 px-4 font-medium">내용</th>
              <th className="py-3 px-4 font-medium">수집시각 (KST)</th>
            </tr>
          </thead>
          <tbody>
            {events.length === 0 ? (
              ...
              events.map((event) => (
                <tr key={event.id} className="border-b border-gray-100">
                  <td className="py-3 px-4 text-xs text-gray-500">{formatKst(event.observedAt)}</td>
                  <td className="py-3 px-4">{event.authorName}</td>
                  <td className="py-3 px-4 max-w-xl break-all">{event.content}</td>
                  <td className="py-3 px-4 text-xs text-gray-500">{formatKst(event.createdAt)}</td>
                </tr>
              ))
            )}
          </tbody>
